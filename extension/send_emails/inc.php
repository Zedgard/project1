...
class send_emails extends \project\extension {
    ...
    public function send($email_code, $to_email, $params = array()) {
        if (strlen($email_code) > 0 && strlen($to_email) > 0) {
            ...
            // Ссылка на быструю авторизацию
            $params['user_auth_fast'] = '/auth/';
            if (count($user_info) > 0 && strlen($user_info['email']) > 0) {
                $auth = new \project\auth();
                $code_integration = $auth->set_code_integration($user_info['email']);
                $params['user_auth_fast_url'] = '/auth/?code_integration=' . $code_integration;
            }
