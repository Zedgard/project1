...
class send_emails extends \project\extension {
    ...
    public function get_email($email_id) {
        $querySelect = "SELECT * FROM `zay_smtp_emails` e WHERE e.id='?' ";
        return $this->getSelectArray($querySelect, array($email_id))[0];
    }
     ...
     * @return boolean
     */
    public function send($email_id = 0, $to_email, $params = array()) {
        if ($email_id > 0 && strlen($to_email) > 0) {
            //echo 1;
            $mail = new PHPMailer(false);
            $user_info = $this->get_smtp_user_info();
            $email_info = $this->get_email($email_id);
